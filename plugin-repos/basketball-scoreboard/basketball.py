...
class Basketball(SportsCore):
    ...
    def _extract_game_details(self, game_event: Dict) -> Optional[Dict]:
        ...
        try:
            ...
            # Extract basketball situation data (possession, bonus, timeouts)
            if situation and status_state == "in":
                possession_team_id = situation.get("possession")
                if possession_team_id:
                    if str(possession_team_id) == str(home_team.get("id")):
                        details["possession_indicator"] = "home"
                    elif str(possession_team_id) == str(away_team.get("id")):
                        details["possession_indicator"] = "away"
                details["home_timeouts"] = situation.get("homeTimeouts", 0)
                details["away_timeouts"] = situation.get("awayTimeouts", 0)
                details["is_bonus"] = situation.get("isBonus", False)
                details["is_double_bonus"] = situation.get("isDoubleBonus", False)
